...
Gemini CLI installs one extension per directory and `gemini extensions install <github-url>` expects the manifest at the install source's root, so for this monorepo, clone and install each extension from its local path.
